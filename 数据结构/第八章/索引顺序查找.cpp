...
int search(int key,int a[])
{
	int i,startValue;
	i=0;
	while(i<3&&key>newIndex[i].key)//确定在哪个块中遍历每一个块确定key在哪个块中 
	{
		i++;
	}
	if(i>=3)//大于分得的块数则返回0 
	...
	if(startValue>startValue+5)//如果大于块的范围结束值则说明没有要查找的数 
	{
		return -1; 
	} 
	return startValue;
...
int main(int argc, char** argv) 
{
	...
	if(k>0)
	{
		printf("查找成功!您想要查找的数在数组中的位置是:%d\n",k+1);
	} 
	else
	{
		printf("查找失败!您想要找的数不在数组中.\n");
	}
	return 0;
}
